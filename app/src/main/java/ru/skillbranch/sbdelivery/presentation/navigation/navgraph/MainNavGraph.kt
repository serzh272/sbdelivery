...
fun MainNavGraph(modifier: Modifier = Modifier, navController: NavHostController) {
    NavHost(navController, startDestination = MainNavigationDestination.Main.route, modifier = modifier) {
        ...
        composable(MainNavigationDestination.Profile.route) { ProfileScreen(navController) }
        composable(MainNavigationDestination.Orders.route) { OrdersScreen(navController) }
        composable(MainNavigationDestination.Notifications.route) { NotificationsScreen(navController) }
        composable(MainNavigationDestination.About.route) { AboutScreen(navController) }
    }
}
...
sealed class MainNavigationDestination(
    ...
    object Favorites : MainNavigationDestination("favorite", R.drawable.ic_favorite_24, R.string.favorites)
    object Cart : MainNavigationDestination("cart", R.drawable.ic_cart_24, R.string.cart)
    object Profile : MainNavigationDestination("profile", R.drawable.ic_profile_24, R.string.profile)
    object Orders : MainNavigationDestination("orders", R.drawable.ic_orders_24, R.string.orders)
    object Notifications : MainNavigationDestination("notifications", R.drawable.ic_notifications_24, R.string.notifications)
